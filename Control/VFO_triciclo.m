function [u1,u2,x,y,cita,ex,ey,ecita,efi,rWheelVel,lWheelVel] = VFO_triciclo(xtp1,ytp1,cita_ir,xr,yr,citar,fir,signmov,vr,kp,k1,dt,ts)
...
for k = 1:tf/dt+1        
    ...
    if( k > 1)
        ...
        [dx,dy,dcita,dfi] = ModeloTriciclo(x(k-1),y(k-1),cita(k-1),fi(k-1),u1(k-1),u2(k-1));
        x(k) = dx*dt + x(k-1); 
        y(k) = dy*dt + y(k-1); 
...
%     L = 2;R = 3;
%     rWheelVel(k) = (2*u2(k) + u1(k)*L)/2*R;
%     lWheelVel(k) = (2*u2(k) - u1(k)*L)/2*R;
end
end
