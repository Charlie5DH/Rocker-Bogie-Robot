...
if(path_x(1) == 0)
    ...
    %esta parte es para marcar una trayectoria fija
%     load('xtp1 ytp1');  
%     path_x = xtp1*Re;
%     path_y = ytp1*Re;
    ...
    Kp = 10;                                      %(1/seg) parametro del controlador
    %K1 = 10;                                    %parametro del controlador para TRM
    k_cita = 15;                                  %parametro del controlador
    ...
    firef = ordenarArreglo(firef);
   
    %[u1,u2,x,y,cita,ex,ey,ecita,efi,rWheelVel,lWheelVel] = VFO_triciclo(path_x,path_y,cita_ini_rob,xref,yref,citaref,firef,signmov,Vr,Kp,K1,dt,ts);
    %[u1,u2,x,y,cita,ex,ey,ecita,efi,rWheelVel,lWheelVel] = VFO_AckermanFWD(path_x,path_y,cita_ini_rob,xref,yref,citaref,firef,signmov,Vr,Kp,k_cita,k_fi,dt,ts,L,wheel_base,wheel_radi);
    [u1,u2,x,y,cita,ex,ey,ecita,rWheelVel,lWheelVel] = VFO_differential(path_x,path_y,cita_ini_rob,xref,yref,citaref,signmov,Vr,Kp,k_cita,dt,ts,wheel_base,wheel_radi);
    ...
    graficar(large,wide,Re,path_x,path_y,dt,ts,xref,yref,x,y,scaleMap,cita,citaref,u1,u2,ex,ey,ecita,rWheelVel,lWheelVel); %GRAFICAR
