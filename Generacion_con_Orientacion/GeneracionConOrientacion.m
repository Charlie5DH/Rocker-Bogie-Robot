function[xref,yref,citaref,firef,kref] = GeneracionConOrientacion(x,y,cita_i,cita_f,dr,vr,dt)
...
if(abs(cita_inicial - cita_i)>1*pi/180 & abs(cita_inicial - cita_i) ~= 2*pi)
    [xref,yref,citaref,firef,kref,m]=GiroOrientacion(x(1),y(1),cita_i,cita_inicial,dr,vr,dt);
...
[xt,yt,citat,fit,kt] = ajustarCurva(xtramo,ytramo,f,cita_inicial,vr,dt,dr);
...
if(abs(cita_llegada_ideal - cita_llegada)< 0.01)
...
cita_llegada = AngRango0a2pi(cita_llegada);
if(abs(cita_llegada - cita_f)>1*pi/180 & abs(cita_llegada - cita_f) ~= 2*pi)
    [xt,yt,citat,fit,kt,m]=GiroOrientacion(x(length(x)),y(length(y)),cita_llegada ,cita_f,dr,vr,dt);
    %copiando valores para los vectores de trayectoria
    offset =length(xref);
    for j=offset+1:offset+length(xt)
        xref(j) = xt(j-offset);
